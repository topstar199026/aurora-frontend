<template>
  <div
    class="modal fade"
    id="modal_add_time_requirements"
    tabindex="-1"
    aria-hidden="true"
    ref="addTimeRequirementsModalRef"
...
export default defineComponent({
  name: "create-time-requirement",
  components: {},
  setup() {
    const store = useStore();
    const formRef = ref(null);
    const addTimeRequirementsModalRef = ref(null);
    ...
    const submit = () => {
      ...
      formRef.value.validate((valid) => {
        if (valid) {
          loading.value = true;
          store
            .dispatch(Actions.APT_TIME_REQUIREMENT.CREATE, formData.value)
            .then(() => {
              loading.value = false;
              ...
              Swal.fire({
                ...
                hideModal(addTimeRequirementsModalRef.value);
              });
            })
            .catch(({ response }) => {
              loading.value = false;
