...
export default class AdminModule extends VuexModule implements AdminInfo {
  ...
  @Mutation
  [Mutations.SET_ADMIN_LIST](adminData) {
    this.adminData = adminData;
  }

  @Mutation
  [Mutations.SET_SELECT_ADMIN](data) {
  ...
  [Actions.LIST_ADMIN]() {
    if (JwtService.getToken()) {
      ...
      ApiService.get("admins")
        .then(({ data }) => {
          this.context.commit(Mutations.SET_ADMIN_LIST, data.data);
          return data.data;
        })
        .catch(({ response }) => {
